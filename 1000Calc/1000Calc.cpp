...
private:
	...
	void mulInt(UINT32 * result, int resultLength, UINT32 * a, int aLength, UINT32 * b, int bLength) {
		if(resultLength != aLength + bLength || aLength<1 || bLength<1) {
			...
			wsprintfW(msg, L"mulInt length error: %d != %d + %d", resultLength, aLength, bLength);
                     ...
                     _In_ int       nCmdShow)
{
    ...
    LoadStringW(hInstance, IDC_MY1000CALC, szWindowClass, MAX_LOADSTRING);
    ...
    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_MY1000CALC));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon          = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_MY1000CALC));
    wcex.hCursor        = LoadCursor(nullptr, IDC_ARROW);
    wcex.hbrBackground  = (HBRUSH)(COLOR_WINDOW+1);
    wcex.lpszMenuName   = MAKEINTRESOURCEW(IDC_MY1000CALC);
